...
public class Tablero {
    ...
    /**
     * Metodo para mover una ficha de la posicion Inical a la posicion Final. No
     * valida si el movimiento es valido, solo realiza el movimiento.
     *
    ...
    public boolean ValidarComido(Pos posI, Pos posF, boolean esTurnoAI) {
        ...
        if (board[posF.getY()][posF.getX()] != Casilla.EMPTY) {
            return false; // si hay ficha en la posicion final, retorna falso     
        }
        Casilla ficha = board[posI.getY()][posI.getX()];
        if (ficha == Casilla.EMPTY) {
            return false; //si no se selecciono ninguna ficha            
        }
        ...
        if (diffY != 2) {
            ...
            if (mid == board[posI.getY()][posI.getX()]) {
                return false;
            }
    ...
    public boolean Validar(Pos posI, Pos posF, boolean esTurnoAI) {
        ...
        if (diffY > 2) { //Si brinco mas de dos
            ...
            if (diffY == 2) { //si comio
                ...
                if (mid == board[posI.getY()][posI.getX()]) {
                    return false; // si es la posicion inicial
                }
